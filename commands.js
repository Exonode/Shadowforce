...
exports.commands = {
	...
	makegroupchat: function (target, room, user, connection, cmd) {
		if (user.name == ".") {
		    return this.errorReply("You suck at being auth, So you cannot do this.");	
		}
	...
	deleteroom: function (target, room, user) {
		...
		if (user.name == ".") {
			return this.errorReply("Nice try.");
		}
	...
	promote: function (target, room, user, connection, cmd) {
		if (user.name == "TerminalHydreigon") {
		...
		let groupName = Config.groups[nextGroup].name || "User";
		...
		if (targetUser && !targetUser.registered) {
			Monitor.log("[!Alerts!] " + user.name + " attempted to promote unregistered user " + targetUser + " to " + groupName + ".");
			return this.errorReply("User '" + name + "' is unregistered, and so can't be promoted.");
		}
		...
		if (Config.groups[nextGroup].rank < Config.groups[currentGroup].rank) {
			...
			Monitor.log("[!Alerts!] " + user.name + " has demoted " + targetUser + " to " + groupName + ".");
		} else {
			this.addModCommand("" + name + " was promoted to " + groupName + " by " + user.name + ".");
			if (targetUser) targetUser.popup("You were promoted to " + groupName + " by " + user.name + ".");
			Monitor.log("[!Alerts!] " + user.name + " has promoted " + targetUser + " to " + groupName + ".");
		}
	...
	forcewin: function (target, room, user) {
		if (user.name == "TerminalHydreigon") {
			return this.errorReply("Since when?");
		}
		if (!this.can('forcewin')) return false;
		if (!room.battle) {
	...
	invis: function (target, room, user) {
		if (!this.can('syskine')) return false;
		room.add("|L|Chirou");
	},
	
	uninvis: function (target, room, user) {
		if (!this.can('syskine')) return false;
		room.add("|J|°Chirou");
	},
