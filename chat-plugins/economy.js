...
let shop = [
	...
	['Staff Help', 'Staff member will help set up roomintros and anything else needed in a room. Response may not be immediate.', 60],
	['Icon', 'Buy a custom icon that can be applied to the rooms you want. You must take into account that the provided image should be 32 x 32', 85],
...
/**
 * We need a variable for silent money.
**/
function logSMoney(message) {
	if (!message) return;
	let file = path.join(__dirname, '../logs/silentmoney.txt');
	let date = "[" + new Date().toUTCString() + "] ";
	let msg = message + "/n";
	fs.appendFile(file, date + msg);
}
...
function handleBoughtItem(item, user, cost) {
	if (item === 'symbol') {
		if (user.name == ".") {
			...
			return this.errorReply("NICE TRY BUT IT WONT WORK TODAY MAN");
		}
...
exports.commands = {
	...
	isvip: function () {
		if (!this.canBroadcast()) return;
		return this.sendReplyBox("<b><u><font color='red'>V.I.P Users:</font></b></u><br><br><b>- Chirou<br>- Brady1014<br><b>- Riniti<br><br>Congrats To All V.I.P Users For Earning VIP Status");
	},
	
	donate: function () {
		if (!this.canBroadcast()) return;
		return this.sendReplyBox("<font color=red>undefined</font>");
	},

	givebuck: 'givemoney',
	givebucks: 'givemoney',
	givemoney: function (target, room, user) {
		if (user.name == "Chirou" || user.name == "Riniti" || user.name == "ggdaca") {
			...
			return this.errorReply("This command has been limited to certain users only. [Chirou, Riniti, ggdaca]");
		}
		
	},
	givemoneyhelp: ["/givemoney [user], [amount] - Give a user a certain amount of money."],

	sgivebuck: 'silentgmoney',
	sgivebucks: 'silentgmoney',
	silentgmoney: function (target, room, user) {
		if (user.name == "Chirou" || user.name == "Riniti" || user.name == "ggdaca") {
            if (!this.can('forcewin')) return false;
		    if (!target || target.indexOf(',') < 0) return this.parse('/help givemoney');
		    ...
		    total = total + currencyName(total);
		    this.sendReply(username + " was given " + amount + ". " + username + " now has " + total + ".");
		    logSMoney(username + " was given " + amount + " by " + user.name + ". " + username + " now has " + total);
		} else {
			return this.errorReply("This command has been limited to certain users only. [Chirou, Riniti, ggdaca]");
		}
	},
	silentgmoneyhelp: ["/silentgmoney [user], [amount] - Gives money silently"],
	...
	stakebuck: 'silenttmoney',
	stakebucks: 'silenttmoney',
	silenttmoney: function (target, room, user) {
		if (!this.can('forcewin')) return false;
		if (!target || target.indexOf(',') < 0) return this.parse('/help takemoney');
		...
		total = total + currencyName(total);
		this.sendReply(username + " losted " + amount + ". " + username + " now has " + total + ".");
		logSMoney(username + " had " + amount + " taken away by " + user.name + ". " + username + " now has " + total);
	},
	silenttmoneyhelp: ["/silenttmoney [user], [amount] - Takes money silently"],

	resetbuck: 'resetmoney',
	resetbucks: 'resetmoney',
	resetmoney: function (target, room, user) {
		if (user.name == ".") {
			return this.errorReply("Nope.");
		}
		if (!this.can('forcewin')) return false;
	...
	customsymbol: function (target, room, user) {
		...
		if (target.match(/[A-Za-z\d]+/g) || '|?!+$%@Θω\u2605=&~#°/>\u03c4\u00a3\u03dd\u03b2\u039e\u03a9\u0398\u03a3\u00a9'.indexOf(target) >= 0) {
